def solve():
    new_list = []
    for _ in range(100):  #다른코드 new_list=[list(map(int,input(),split())) for _ in range(100)]
    ...
    for h in range(100):
        ...
        for k in range(100):
            sum_c += new_list[k][h]
            if sum_c > max_c:   #다른 코드 max(max_c,sum_c) #max_c = max_c if max_c>sum_C else sum_c
                max_c = sum_c
    max_r = 0  # 행 최대값 구하기
    for k in new_list:
        sum_r = 0
